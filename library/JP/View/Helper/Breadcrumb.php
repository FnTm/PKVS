    ...
    public function breadcrumb (array $breadcrumb = array())
    {
        if (empty($this->_breadcrumb)) {
            if (! empty($breadcrumb)) {
                ...
                $moduleArray = array('registry' => 'Reģistrs');
                $actionArray = array('approve' => 'Apstripinājums','add'=>'Pievienot','all'=>'Visas','mistakes'=>'Visas');
                $controllerArray = array('mistake' => 'Kļūdas');
                $moduleTitle = $module;
                $actionTitle = $action;
                $controllerTitle = $controller;
                if (isset($moduleArray[$module])) {
                    $moduleTitle = $moduleArray[$module];
                }
                if (isset($actionArray[$action])) {
                    $actionTitle = $actionArray[$action];
                }
                if (isset($controllerArray[$controller])) {
                    $controllerTitle = $controllerArray[$controller];
                }
                else{
                    $controllerTitle = ucfirst($controller);
                }
                if ($module != 'default') {
                    $this->_breadcrumb[] = array('title' => $moduleTitle,
                    'url' => $this->view->url(array('module' => $module),
                    'default', true));
                }
                if ($controller != 'index') {
                ...
                if ($action != 'index') {
                    $this->_breadcrumb[] = array('title' => $actionTitle,
                    'url' => $this->view->url(
                    array('module' => $module, 'controller' => $controller,
    ...
    public function __toString ()
    {
        if (count($this->_breadcrumb) == 1) {
            ...
            foreach ($this->_breadcrumb as $i => $bc) {
                $breadcrumb .= '<li>' .
                 ($i != 0 ? '<span>' . $this->_separator . '</span>' : null);
                if ($bc['url'] === null) {
                    $breadcrumb .= $this->view->escape($bc['title']);
                } else {
                    $breadcrumb .= '<a href="' . $bc['url'] . '">' .
                     $this->view->escape($bc['title']) . '</a>';
                }
                $breadcrumb .= '</li>';
            }
            $breadcrumb .= '</ol><br/>';
